        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Edit Car Details</h3>
                    ...
                    <form method="POST" action="{{ route('cars.update', $car) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="generation" id="generation"
                                   value="{{ old('generation', $car->generation) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            @error('generation')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            <label for="version" class="block text-sm font-medium text-gray-700">Version (e.g., 2.0
                                Sport, GT)</label>
                            <input type="text" name="version" id="version" value="{{ old('version', $car->version) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            @error('version')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
